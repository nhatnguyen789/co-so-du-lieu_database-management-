...
--câu 1: với 1 mã sinh viên và 1 mã khoa, kiểm tra xem sv có thuộc khoa này không( trả về có hoặc không)
create function cau1(@maSV varchar(10),@makhoa varchar(10))
returns int 
as 
begin 
                declare @ktra varchar(10)
				select @ktra = maKhoa
				from LOP L,SINHVIEN SV
				where SV.maLop =L.ma AND SV.ma =@maSV
				if(@ktra = @maKhoa) 
				return 1
				else
				return 0
...
--câu 2: tính điểm thi sau cùng của một SV trong trong cùng một môn học cụ thể 
create function  cau2 (@maSV varchar(10),@maMH varchar(10))
returns float 
as
begin 
                 declare @diem float 
				 ...
				 select @diem =KQ.diem
				 from KETQUA KQ
				 where	KQ.maSinhVien = @maSV AND KQ.maMonHoc = @maMH AND 
				 KQ.lanthi =(select Max(K.lanthi)
				 from KETQUA K
				 WHERE K.maSinhVien =@maSV AND K.maMonHoc =@maMH)
return @diem
--câu 3:tính điểm tb của một sinh viên( chú ý;điểm tb được tính  dựa trên lần thi sau cùng) sử dụng function ở câu 2 đã viết 
create function cau3 (@maSV varchar(10))
returns float
as
begin 
                 declare @TB float
                 select @TB=AVG(KQ.Diem.diemtb)
                from  (select dbo.cau2(@maSV,KQ.maMonHoc) as diemtb
				 from KETQUA KQ 
				 where KQ.maSinhVien =@maSV) as KQDiem
return @TB
end
--câu 4: nhập vào 1 sv và 1 môn học, trả về các điểm thi của sv này trong các lần thi môn học đó 
create function cau4(@mssv varchar(10), @maMH varchar(10))
returns table 
as
return( select KQ.lanthi, KQ.diem
      from KETQUA KQ
	  where KQ.mssv =@mssv
	  and KQ.maMonHoc =@maMH

	  select * from dbo.cau4('0212001','THT01')

--câu 5: nhập vào 1 sv trả về danh sách các môn học mà sv này phải học 
create function intenmonhoc(@mssv char(10))
return table
as
return ( select M.intenmonhoc
         from SINHVIEN SV, MONHOC M,LOP L
		 where SV.ma = @mssv and SV.maLop =L.ma 
		 and L.maKhoa =M.maKhoa)
select *from dbo.intenmonhoc ('0212001')
